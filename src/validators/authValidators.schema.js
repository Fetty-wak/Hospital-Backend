...
const baseSchema = z.object({
  ...
  password: z.string(),
  role: z.enum(['DOCTOR', 'PATIENT', 'LAB_TECH', 'ADMIN'])
}).strict();
...
const patientSchema = z.object({
  ...
  allergies: z.string().max(500).nullable().transform(val => val?.trim()),
  gender: z.enum(['MALE', 'FEMALE', 'OTHER']),
  bloodType: z.enum(['A_POS', 'A_NEG', 'B_POS', 'B_NEG', 'AB_POS', 'AB_NEG', 'O_POS', 'O_NEG']),
  dateOfBirth: z.string().refine(val => !isNaN(Date.parse(val)), { message: "Invalid date" }) // ISO date string
}).strict();
